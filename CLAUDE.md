...
- Deploy: Vercel (`vercel.json` com CSP para Google Auth e endpoints de FCM em
  `connect-src`). Config `.replit` para Replit.
  - **Login Google = `signInWithPopup`** como via primÃ¡ria; `signInWithRedirect` Ã© fallback quando o popup Ã© bloqueado.
    `Cross-Origin-Opener-Policy: same-origin-allow-popups` permite o popup; o redirect fica como
    fallback.
    Ver `AuthPage.js` (usa `getRedirectResult` para processar esse fallback).
- Há testes unitários de domínio em `src/domain/**/*.test.js`; a cobertura ainda é parcial.
...
| Camada | Arquivo |
|---|---|
| Domain | `src/domain/services/WeeklyChallengeState.js` |
| Application | — (ainda há lógica de orquestração na presentation) |
| Presentation | `src/presentation/components/DailyChallenge.js` (transações Firestore para aceitar, reivindicar e confirmar) |
...
| `photoURL` | string? | ID do catálogo de avatares ou URL legacy |
...
| `createdBy` / `invitedUserId` | uid | Metadados imutáveis da vinculação, usados pelas regras |
...
Desafio semanal — escrito por transações em `DailyChallenge.js`, com merge puro em
`WeeklyChallengeState.js`. Campos principais: `challengeId`, `acceptedBy`, `acceptedAt`,
`weekStartDate`, `state`, `confirmations.{uid}`.
...
Codigo de convite de 6 caracteres. `creatorId`, `creatorNickname`, `coupleId`, `createdAt`. Deletado ao consumir.
...
(ao notificar com app fechado). As regras estão versionadas em `firestore.rules`; o cliente
só pode gravar o próprio documento e manter o `uid` coerente.
...
As Cloud Functions usam o SDK admin e ignoram regras para leitura/envio; os tokens inválidos
são removidos pelo emissor.
...
> **Correções desta tarefa:** atividades só marcam `pointsAwarded` quando ambos concluíram;
> regras cíclicas são avaliadas mesmo sem atividades; o desafio semanal usa transações e
> preserva o estado do parceiro; desvinculação limpa `onboardingSkipped` dos dois usuários;
> regras do Firestore restringem perfis, convites, tokens e criação de casais.
...
- `vercel.json` define CSP para Google Auth (popup e redirect de fallback) + conecta-se aos
