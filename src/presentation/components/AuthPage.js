...
function AuthPage() {
  ...
  // Preenchido só quando alguém entra pelo Google e ainda não tem
  // documento de usuário no Firestore — precisamos pedir apelido/gênero
  // antes de considerar o cadastro concluído, igual ao fluxo de e-mail.
   ...
   * precisa de mais nada aqui. Se é a primeira vez dessa pessoa, ainda
   * faltam apelido e gênero (o Google não fornece isso), então pedimos
   * numa etapa curta antes de considerar o cadastro concluído.
   */
  ...
  const handleFinishGoogleSignup = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", pendingGoogleUser.uid), initialUserData);
      setNewUserData({ nickname, gender, email: pendingGoogleUser.email });
      setPendingGoogleUser(null);
    } catch (err) {
      setError("Não foi possível concluir o cadastro. Tente novamente.");
      console.error("Erro ao concluir cadastro com Google:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-800/50 border border-gray-700/50 rounded-3xl shadow-glow-accent p-8 space-y-6 backdrop-blur-sm relative">
        ...
        {pendingGoogleUser ? (
          <form onSubmit={handleFinishGoogleSignup} className="space-y-4">
            <input
              type="text"
              placeholder="Apelido"
              value={nickname}
